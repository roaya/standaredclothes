...
#Region "DataMethods"
    Private Sub DataClear()
        cmbAgeID.SelectedIndex = -1
        QueryAllItems.Rows.Clear()

    End Sub
    Private Sub DataLoad()
        cmd.CommandText = "select age_desc from ages"
        dr = cmd.ExecuteReader
        Do While Not dr.Read = False
            cmbAgeID.Items.Add(dr("age_desc"))
        Loop
        dr.Close()
    End Sub

    Private Function DataValidate(errorMessage As String) As Boolean
        If cmbAgeID.SelectedIndex = -1 Then
            DataValidate = False
...
#End Region
    ...
    Private Sub choseForm()
        selformula = ""
        If cmbAgeID.SelectedIndex <> -1 Then
            selformula = "select Category_Name AS 'اسم البند',Item_Name AS 'اسم الصنف',Stock_Name AS 'اسم المحل',Purchase_Price AS 'سعر الشراء',Sale_Price AS 'سعر البيع',Sale_Total_Price AS 'سعر الجملة', Corporation_Name AS 'اسم الشركة',Gender_Name AS 'النوع',From_Age AS 'من سن',To_Age AS 'الي سن',Size_Name AS 'المقاس' FROM Query_All_Items where Age_Desc=N'" & (cmbAgeID.Text) & "'"

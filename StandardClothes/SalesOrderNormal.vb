    ...
    Sub Commit_Form()
        Try
            ...
            ElseIf PayedValue.Value = 0 And PayType.Text <> "اجل" Then
                cls.MsgExclamation("ادخل قيمة المدفوع")
            ElseIf PayedValue.Value <> TotalBill.Text And PayType.Text = "نقدي" Then
                cls.MsgExclamation("يجب ان لا يقل اجمالى المدفوع عن اجمالى الفاتوره")
            ElseIf CustomerID.Text = "" Then
                cls.MsgExclamation("اختر اسم العميل")
...
#End Region
    ...
    Private Sub CashValue_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CashValue.ValueChanged
        If PayType.Text = "نقدي و اجل" Then
            CreditValue.Value = TotalBill.Text - CashValue.Value

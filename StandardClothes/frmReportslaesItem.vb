Public Class frmReportslaesItem
    Dim Gcls As GeneralSp.MasterForms
    Dim B_EndLoad As Boolean = False
    Dim ITEM_NAME As String
    Dim selformula As String
    Dim ReportslaesItemtabel As New DataTable
    '-------------------------------
    'Set The Data Table Name
    Dim TName As String = "ReportslaesItemtabel"
    Dim RptExpenses As New Report_slaes_Item
...
#Region "DataMethods"
    Private Sub DataClear()
        ...
        ReportslaesItemtabel.Rows.Clear()
...
#End Region
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        da.Fill(ReportslaesItemtabel)
        If ReportslaesItemtabel.Rows.Count > 0 Then

            RptExpenses.SetDataSource(MyDs.Tables("ReportslaesItemtabel"))
    ...
    Private Sub choseForm()
        ...
        If CmbVendor.SelectedIndex <> -1 Then
            selformula = "select * from Report_slaes where Vendor_Name=N'" & (CmbVendor.Text) & "'"

        End If

        If cmbCat.SelectedIndex <> -1 Then
            selformula = "select * from Report_Sales where Category_Name=N'" & (cmbCat.Text) & "'"
        End If

        If cmbItem.SelectedIndex <> -1 Then
            selformula = "select * from Report_Sales where Item_Name=N'" & (cmbItem.Text) & "'"
        End If

        If RadDate.Checked = True Then
            Dim date1 As Date = CDate(DateTimePicker1.Text)
            Dim date2 As Date = CDate(DateTimePicker2.Text)
            selformula = "select * from Report_Sales where Bill_Date between '" & date1.ToString("MM/dd/yyyy") & "'  and   '" & date2.ToString("MM/dd/yyyy") & "'"
        End If
    End Sub
